    ...
    class PaymentAPI
    {
        function getPaymentHistory($json)
        {
            ...
            $role = isset($json['role']) ? $json['role'] : 'frontdesk';
            $page = isset($json['page']) ? intval($json['page']) : 1;
            $limit = isset($json['limit']) ? intval($json['limit']) : 10;
            $offset = ($page - 1) * $limit;

            // Build role filter
            $roleFilter = '';
            if ($role === 'frontdesk') {
                $roleFilter = " AND ur.role_type = 'front desk' ";
            }
            // Select all payment history details with correct columns
            $sql = "SELECT ph.history_id, ph.payment_id, ph.billing_id, ph.reservation_id, ph.changed_by_user_id, ph.changed_at,
            ...
            LEFT JOIN User u ON ph.changed_by_user_id = u.user_id
            LEFT JOIN UserRoles ur ON u.user_roles_id = ur.user_roles_id
            LEFT JOIN Billing b ON ph.billing_id = b.billing_id
            LEFT JOIN Reservation res ON ph.reservation_id = res.reservation_id
            ...
            WHERE p.is_deleted = 0" . $roleFilter . "
        ...
        VALUES (:user_id, :billing_id, :reservation_id, :sub_method_id, :amount_paid, :payment_date, :notes, :reference_number, :proof_of_payment_url, :money_given, :change_given)";
            ...
            try {
                ...
                if ($stmt->rowCount() > 0) {
                    ...
                    $sqlHistory = "INSERT INTO PaymentHistory (payment_id, billing_id, reservation_id, changed_by_user_id, changed_at) VALUES (:payment_id, :billing_id, :reservation_id, :changed_by_user_id, NOW())";
                    $stmtHistory = $db->prepare($sqlHistory);
                    $stmtHistory->bindParam(":payment_id", $payment_id);
                    $stmtHistory->bindParam(":billing_id", $json['billing_id']);
                    $stmtHistory->bindParam(":reservation_id", $json['reservation_id']);
                    $stmtHistory->bindParam(":changed_by_user_id", $user_id);
                    $stmtHistory->execute();
                }
        ...
        function updatePayment($json)
        {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $stmtHistory->bindParam(":billing_id", $json['billing_id']);
                $stmtHistory->bindParam(":reservation_id", $json['reservation_id']);
                $stmtHistory->bindParam(":changed_by_user_id", $json['user_id']);
                $stmtHistory->execute();
            }
            echo json_encode($returnValue);
        }
        ...
        function deletePayment($json)
        {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $stmtHistory->bindParam(":billing_id", $json['billing_id']);
                $stmtHistory->bindParam(":reservation_id", $json['reservation_id']);
                $stmtHistory->bindParam(":changed_by_user_id", isset($json['user_id']) ? $json['user_id'] : null);
                $stmtHistory->execute();
            }
            echo json_encode($returnValue);
        }
    ...
    switch ($operation) {
        ...
        case "getPaymentHistory":
            $payment->getPaymentHistory($json);
            break;
    }
